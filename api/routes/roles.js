...
router.post("/add", async (req, res) => {
    ...
    try {
        ...
        if(!body.permissions || !Array.isArray(body.permissions) || body.permissions.length == 0){
            throw new CustomError(Enum.HTTP_CODES.BAD_REQUEST, "Validation Error", "permissions filed must be in an array"); 
        }
        let role = new Roles({
...
router.post("/update", async (req, res) => {
    ...
    try{
        ...
        if(body.permissions || Array.isArray(body.permissions) || body.permissions.length > 0){
            ...
            //Postmande permissions.filter is not a function hatası alıyoruz. Çözümünü bulamadık ve burda kaldık.
